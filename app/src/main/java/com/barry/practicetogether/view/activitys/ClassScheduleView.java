...
//    private String dateTime = "2018年07月23日 星期一 第23周";
     ...
     * 星期行数据
     */
    private String rows[] = {"一", "二", "三", "四", "五", "六", "日"};
    /**
     * 星期行标记
     */
     ...
     * 节列数据
     */
    private String columns[] = {"09:00", "09:30", "10:00", "10:30", "11:00", "11:30", "12:00", "12:30", "13:00", "13:30", "14:00", "14:30", "15:00", "15:30", "16:00", "16:30", "17:00", "17:30", "18:00", "18:30", "19:00", "19:30", "20:00"};
...
//    }
    ...
    private void drawView(Canvas canvas) {
        ...
        itemHeight = viewHeight / columnCount;
        //返回按钮
...
//        canvas.drawText(dateTime, getLeft() * 2 + viewWidth - titleDateSize - titleRightPadding - getLeft(), getTop() + (isLandscape() ? (titleHeight / 2 + singleTitle / 3) : (titleHeight - dpToPx(5))), titleDatePaint);
        ...
        for (int row = 0; row < rowCount + 1; row++) {
            for (int column = 0; column < columnCount + 1; column++) {
                ...
                canvas.drawLine(getLeft() + 0, getTop() + column * itemHeight, getLeft() + viewWidth, getTop() + column * itemHeight, rowPaint);
            }
            //竖线
            Paint columnPaint = new Paint();
            columnPaint.setColor(dividerColor);
            columnPaint.setStrokeWidth(2F);
            canvas.drawLine(getLeft() + row * itemWidth, getTop(), getLeft() + row * itemWidth, getTop() + viewHeight, columnPaint);

        }
        //课程表分割线
//        Paint titlePaint = new Paint();
//        titlePaint.setColor(titleDividerColor);
//        canvas.drawLine(getLeft() + 0, getTop(), getLeft() + viewWidth, getTop(), titlePaint);
        //边框线
...
//        canvas.drawRoundRect(rectF, null, null, borderlinesPaint);
    }
...
//    }
    ...
    private void initWHCoordinates(ItemOptions options, int row, int column) {
        ...
        options.setMoveY(getTop() + itemHeight * column);
        options.setMinCoordinates(getLeft() + itemWidth * row, getTop() + itemHeight * column);
        options.setMaxCoordinates(getLeft() + itemWidth + itemWidth * row, getTop() + itemHeight + itemHeight * column);
    }
